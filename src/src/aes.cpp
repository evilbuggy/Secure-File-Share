#include "aes.h"

static const uint8_t sbox[256] = {
	
};

static const uint8_t invsbox[256] = {
	
};

static const uint8_t RC[10] = {
	0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

static void subBytes(uint8_t state[4][4]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] = sbox[state[i][j]];
		}
	}
}

static void invSubBytes(uint8_t state[4][4]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] = invsbox[state[i][j]];
		}
	}
}

static void shiftRows(uint8_t state[4][4]) {
	uint8_t tmp[4][4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			tmp[i][j] = state[i][j];
		}
	}
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] = tmp[i][(i + j) % 4];
		}
	}
}

static void invShiftRows(uint8_t state[4][4]) {
	uint8_t tmp[4][4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			tmp[i][j] = state[i][j];
		}
	}
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] = tmp[i][(4 + j - i) % 4];
		}
	}
}

static uint8_t multiply(uint8_t x, uint8_t y) {
	int p = x, q = y;
	int cnt = 0;
	for (int i = 7; i >= 0; i--) {
		if ((1 << i)&q) {
			cnt ^= (p << i);
		}
	}
	for (int i = 15; i >= 8; i--) {
		if ((1 << i)&cnt) {
			cnt ^= ((0x1b) << (i - 8));
		}
	}
	return uint8_t(cnt);
}

static void mixColumns(uint8_t state[4][4]) {
	uint8_t tmp[4][4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			tmp[i][j] = state[i][j];
		}
	}
	for (int j = 0; j < 4; j++) {
		state[0][j] = multiply(0x02, tmp[0][j]) ^ multiply(0x03, tmp[1][j]) ^ tmp[2][j] ^ tmp[3][j];
		state[1][j] = multiply(0x02, tmp[1][j]) ^ multiply(0x03, tmp[2][j]) ^ tmp[3][j] ^ tmp[0][j];
		state[2][j] = multiply(0x02, tmp[2][j]) ^ multiply(0x03, tmp[3][j]) ^ tmp[0][j] ^ tmp[1][j];
		state[3][j] = multiply(0x02, tmp[3][j]) ^ multiply(0x03, tmp[0][j]) ^ tmp[1][j] ^ tmp[2][j];
	}
}

static void invMixColumns(uint8_t state[4][4]) {
	uint8_t tmp[4][4];
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			tmp[i][j] = state[i][j];
		}
	}
	for (int j = 0; j < 4; j++) {
		state[0][j] = multiply(0x0e, tmp[0][j]) ^ multiply(0x0b, tmp[1][j]) ^ multiply(0x0d, tmp[2][j]) ^ multiply(0x09, tmp[3][j]);
		state[1][j] = multiply(0x09, tmp[0][j]) ^ multiply(0x0e, tmp[1][j]) ^ multiply(0x0b, tmp[2][j]) ^ multiply(0x0d, tmp[3][j]);
		state[2][j] = multiply(0x0d, tmp[0][j]) ^ multiply(0x09, tmp[1][j]) ^ multiply(0x0e, tmp[2][j]) ^ multiply(0x0b, tmp[3][j]);
		state[3][j] = multiply(0x0b, tmp[0][j]) ^ multiply(0x0d, tmp[1][j]) ^ multiply(0x09, tmp[2][j]) ^ multiply(0x0e, tmp[3][j]);
	}
}

static void expandKey(uint8_t key[16], uint8_t roundKey[4][44]) {
	for (int i = 0; i < 16; i++) {
		roundKey[(i % 4)][(i / 4)] = key[i];
	}
	uint8_t tmp[4];
	for (int i = 4; i < 44; i++) {
		for (int j = 0; j < 4; j++) {
			tmp[j] = roundKey[j][i - 1];
		}
		if (i % 4 == 0) {
			uint8_t temp = tmp[0];
			tmp[0] = tmp[1];
			tmp[1] = tmp[2];
			tmp[2] = tmp[3];
			tmp[3] = temp;
			for (int j = 0; j < 4; j++) {
				tmp[j] = sbox[tmp[j]];
			}
			tmp[0] ^= RC[(i / 4) - 1];
		}
		for (int j = 0; j < 4; j++) {
			roundKey[j][i] = tmp[j] ^ roundKey[j][i - 4];
		}
	}
}

static void addRoundKey(uint8_t state[4][4], int round, uint8_t roundKey[4][44]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] ^= roundKey[i][4 * round + j];
		}
	}
}

static void initState(uint8_t state[4][4], uint8_t txt[16]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			state[i][j] = txt[4 * j + i];
		}
	}
}

static void deductTxt(uint8_t state[4][4], uint8_t txt[16]) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			txt[4 * j + i] = state[i][j];
		}
	}
}

void aes_encrypt(uint8_t ptxt[16], uint8_t ctxt[16], uint8_t key[16]){
	uint8_t roundKey[4][44];
	expandKey(key, roundKey);

	uint8_t state[4][4];
	initState(state, ptxt);

	int round = 0;
	addRoundKey(state, round++, roundKey);

	for (; round < 10; round++) {
		subBytes(state);
		shiftRows(state);
		mixColumns(state);
		addRoundKey(state, round, roundKey);
	}

	subBytes(state);
	shiftRows(state);
	addRoundKey(state, round, roundKey);

	deductTxt(state, ctxt);
}

void aes_decrypt(uint8_t ptxt[16], uint8_t ctxt[16], uint8_t key[16]){
	uint8_t roundKey[4][44];
	expandKey(key, roundKey);

	uint8_t state[4][4];
	initState(state, ctxt);

	int round = 10;
	addRoundKey(state, round--, roundKey);

	for (; round > 0; round--) {
		invShiftRows(state);
		invSubBytes(state);
		addRoundKey(state, round, roundKey);
		invMixColumns(state);
	}

	invShiftRows(state);
	invSubBytes(state);
	addRoundKey(state, round, roundKey);

	deductTxt(state, ptxt);
}

void aes_cbc_encrypt(uint8_t ptxt[16], uint8_t ctxt[16], uint8_t key[16], uint8_t iv[16]){
    uint8_t tmp[16];
    for(int i = 0; i < 16; i++){
        tmp[i] = ptxt[i] ^ iv[i];
    }
    aes_encrypt(tmp, ctxt, key);
    for(int i = 0; i < 16; i++){
        iv[i] = ctxt[i];
    }
}

void aes_cbc_decrypt(uint8_t ptxt[16], uint8_t ctxt[16], uint8_t key[16], uint8_t iv[16]){
    uint8_t tmp[16];
    aes_decrypt(tmp, ctxt, key);
    for(int i = 0; i < 16; i++){
		ptxt[i] = tmp[i] ^ iv[i];
    }
    for(int i = 0; i < 16; i++){
        iv[i] = ctxt[i];
    }
}
